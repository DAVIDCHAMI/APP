...
public class PagosBilleteraStepDefinition {
  ...
  public void actorIngresaBilleteraParaRegistrarse(String tipoTransaccion) {
    theActorInTheSpotlight()
        .attemptsTo(SeleccionarOpcion.delMenu(tipoTransaccion), RegistrarBilletera.enApp());
  }
  ...
  public void realizaActivacionBilletera(List<Map<String, String>> informacionBilletera) {
    theActorInTheSpotlight()
        .attemptsTo(
            ...
            PrepararPagoConBilletera.conQrEstatico(
                informacion().dePagoBilletera(informacionBilletera)),
            RealizarPago.conBilletera());
  }

  @Y("^realiza una compra con QR estatico cambiando el valor del pago inicial$")
  public void realizaCompraBilleteraCambiandoValores(
      List<Map<String, String>> informacionBilletera) {
    theActorInTheSpotlight()
        .attemptsTo(
            LeerQR.estatico(informacion().dePagoBilletera(informacionBilletera)),
            PrepararPagoConBilletera.conQrEstatico(
                informacion().dePagoBilletera(informacionBilletera)),
            Cambiar.valorPagarQrEstatico(
                informacion().conValor(informacionBilletera).conPropina(informacionBilletera)),
            RealizarPago.conBilletera());
  }

  @Y("^realiza una compra con QR estatico cambiando la tarjeta$")
  public void realizaCompraBilleteraCambiandoTarjeta(
      List<Map<String, String>> informacionBilletera) {
    theActorInTheSpotlight()
        .attemptsTo(
            LeerQR.estatico(informacion().dePagoBilletera(informacionBilletera)),
            PrepararPagoConBilletera.conQrEstatico(
                informacion().dePagoBilletera(informacionBilletera)),
            Cambiar.tarjetaEnBIlleteraParaPago(informacion().conDatosTarjeta(informacionBilletera)),
            RealizarPago.conBilletera());
  }
  ...
  public void deberiaDeVerMensajePagoBilleteraExitoso() {
    theActorInTheSpotlight()
        .should(
            seeThat(PagoExitoso.realizadoConBilletera())
                .orComplainWith(
                    NoSeRealizoPagoQrEstaticoBilleteraException.class,
                    MENSAJE_PAGO_NO_REALIZADO_EXCEPTION));
  }
}
