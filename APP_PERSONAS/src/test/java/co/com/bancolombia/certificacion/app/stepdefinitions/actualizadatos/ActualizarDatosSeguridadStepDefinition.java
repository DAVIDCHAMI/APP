...
public class ActualizarDatosSeguridadStepDefinition {
  ...
  public void modificaDatosSeguridadConInformacion(List<Map<String, String>> datos) {
    theActorInTheSpotlight()
        .attemptsTo(
            ActualizarDatosSeguridad.conInformacion(
                usuario().conCorreo(datos).conTipoCorreo(datos).conNumeroCelular(datos)));
  }
  ...
  public void deberiaVerDatosActualizados() {
    theActorInTheSpotlight()
        .should(
            seeThat(VerificarActualizacionDatosSeguridad.exitosa())
                .orComplainWith(NoSeActualizaronDatosException.class, ACTUALIZACION_FALLODA));
  }
}
