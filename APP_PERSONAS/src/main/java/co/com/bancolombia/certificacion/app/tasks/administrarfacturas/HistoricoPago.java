...
public class HistoricoPago implements Task {
  ...
  public <T extends Actor> void performAs(T actor) {
    actor.attemptsTo(
        Saltar.onBoarding(),
        Check.whether(!opcion.equalsIgnoreCase(SELECCIONAR_TODAS))
            .andIfSo(SeleccionarOpcionFactura.conInformacion(OPT_VER_HISTORICO_FACTURA, factura)),
    ...
    while (Verificar.elementoVisible(
        actor, LBL_CONTENEDOR_HISTORICO_PAGO.of(String.valueOf(iterador)))) {
      listaHistoricoPago.add(
          factura()
              .conReferencia(
                  LBL_REFERENCIA_HISTORICO_PAGO
                      .of(String.valueOf(iterador))
                      .resolveFor(actor)
                      .getText())
              .conFechaFactura(
                  LBL_FECHA_HISTORICO_PAGO.of(String.valueOf(iterador)).resolveFor(actor).getText())
              .conEstadoHistorico(
                  LBL_ESTADO_HISTORICO_PAGO
                      .of(String.valueOf(iterador))
                      .resolveFor(actor)
                      .getText())
              .conValor(
                  LBL_VALOR_HISTORICO_PAGO.of(String.valueOf(iterador)).resolveFor(actor).getText())
              .build());
      iterador++;
    }
