...
public class Propia implements Task {
  ...
  public <T extends Actor> void performAs(T actor) {
    actor.attemptsTo(
        Check.whether(elementoVisible(actor, TXT_TARJETAS))
            .andIfSo(
                Validar.carga(),
                RealizarScroll.hastaPosicionDeTarget(
                    BTN_TARJETA_CREDITO_PAGAR.of(
                        tarjetaCredito.getTipo(), tarjetaCredito.getNumero())),
                Click.on(
                    BTN_TARJETA_CREDITO_PAGAR.of(
                        tarjetaCredito.getTipo(), tarjetaCredito.getNumero()))),
        ...
        Check.whether(OTRO_VALOR.equals(tarjetaCredito.getTipoPago()))
            .andIfSo(
                Check.whether(!tarjetaCredito.getTipo().toLowerCase().contains(VISA.toLowerCase()))
                    .andIfSo(Click.on(CHK_MONEDA.of(tarjetaCredito.getMoneda()))),
        ...
        RealizarScroll.hastaPosicionDeTarget(
            BTN_PRODUCTO_ORIGEN.of(productoDebitar.getTipo(), productoDebitar.getNumero())),
