...
public class ConsultarExistenciaCliente implements Question<Boolean> {
  ...
  public Boolean answeredBy(Actor actor) {
    ...
    if (!registros.isEmpty()) {
      ...
      resultadoDato =
          validarCampo(
              ConstantesIseries.DOCUMENTO,
              registros.get(0).get("cnnoss").toString().trim(),
              StringManager.formatoDocumento(transaccion.getUsuario().getNumeroDocumento()),
              resultadoDato);
      resultadoDato =
          validarCampo(
              ConstantesIseries.TIPODOCUMENTO,
              registros.get(0).get("cncdti").toString().trim(),
              transaccion.getUsuario().getTipoDocumento(),
              resultadoDato);
      resultadoDato =
          validarCampo(
              ConstantesIseries.ESTADOUSUARIO,
              registros.get(0).get("estado").toString().trim(),
              "A",
              resultadoDato);
      ...
      Serenity.recordReportData()
          .withTitle("Verificacion Back Autenticacion")
          .andContents(Serenity.sessionVariableCalled("log"));
    } else {
