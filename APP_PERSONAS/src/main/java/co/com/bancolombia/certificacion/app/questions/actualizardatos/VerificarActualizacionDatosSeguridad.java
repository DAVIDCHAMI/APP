...
public class VerificarActualizacionDatosSeguridad implements Question<Boolean> {
  ...
  public Boolean answeredBy(Actor actor) {
    ...
    return (Visibility.of(LBL_ACTULIZACION_EXITOSA).viewedBy(actor).asBoolean()
        && Visibility.of(LBL_VERIFICACION_ACTUALIZACION_DATOS.of(CORREO_CLAVE_DINAMICA, correo))
            .viewedBy(actor)
            .asBoolean()
        && Visibility.of(
                LBL_VERIFICACION_ACTUALIZACION_DATOS.of(TIPO_CORREO_CLAVE_DINAMICA, tipoCorreo))
            .viewedBy(actor)
            .asBoolean()
        && Visibility.of(LBL_VERIFICACION_ACTUALIZACION_DATOS.of(NUMERO_CELULAR, numeroCelular))
            .viewedBy(actor)
            .asBoolean());
  }
