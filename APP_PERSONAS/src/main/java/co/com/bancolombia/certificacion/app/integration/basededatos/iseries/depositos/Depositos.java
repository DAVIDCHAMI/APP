...
public class Depositos {
  ...
  public static List<Map<String, Object>> saldoDepositosConsolidado(Actor actor) {
    ...
    String sql =
        QueryManager.CONSULTAS.getString("SQL.SCIFFSALDO.consultaSaldoPertenenciaCuentaDeposito");
    return Consulta.ejecutar(sql, dataForQuery, ConnectionManager.getIseriesConnection());
  }
  ...
  public static void saldoDepositosAntes(Actor actor) {
    ...
    String sql = QueryManager.CONSULTAS.getString("SQL.SCIFFSALDO.consultarSaldo");
    List<Map<String, Object>> resultadoConsulta =
        Consulta.ejecutar(sql, dataForQuery, ConnectionManager.getIseriesConnection());
    saldoDisponible = resultadoConsulta.get(0).get("sdsdodsp").toString();
    saldoCanje = resultadoConsulta.get(0).get("sdfltdsp").toString();
    saldoTotal =
        Double.toString(Double.parseDouble(saldoDisponible) + Double.parseDouble(saldoCanje));
  ...
  public static boolean validarDebitoDeposito(
      String saldoAntes, String saldoDespues, String orientacion, String valor) {
