...
public class EditarProgramacion implements Task {
  ...
  public <T extends Actor> void performAs(T actor) {
    actor.attemptsTo(
        ...
        Validar.carga(),
        Click.on(FOCO),
        Check.whether(PRODUCTO_ORIGEN.equals(opcion))
            .andIfSo(
                ...
                RealizarScroll.hastaPosicionDeTarget(
                    OPT_CUENTA_PRODUCTO.of(
                        programarFacturas.getProducto().getTipo(),
                        programarFacturas.getProducto().getNumero())),
                Click.on(
                    OPT_CUENTA_PRODUCTO.of(
                        programarFacturas.getProducto().getTipo(),
                        programarFacturas.getProducto().getNumero()))));
    if (PERIODICIDAD.equals(opcion)) {
      actor.attemptsTo(
          ...
          Validar.carga());
      actor.attemptsTo(
          WaitUntil.the(LST_NUMERO_INTENTOS, isVisible()), Click.on(LST_NUMERO_INTENTOS));
      actor.attemptsTo(
      ...
      while (!Verificar.elementoVisible(
          actor, LBL_MES.of(programarFacturas.getMesProgramacion()))) {
        actor.attemptsTo(Click.on(BTN_FLECHA_MES_SIGUIENTE));
      }
      String valorMes =
          LBL_VALOR_MES.of(programarFacturas.getMesProgramacion()).resolveFor(actor).getValue();
      actor.attemptsTo(
    ...
    actor.attemptsTo(Validar.carga(), Click.on(BTN_MODIFICAR));
    Validar.carga();
  }
}
