...
public class ConsultarPertenenciaTarjetasCredito implements Question<Boolean> {
  ...
  private static final Logger LOGGER =
      LogManager.getLogger(ConsultarPertenenciaTarjetasCredito.class);
  ...
  public Boolean answeredBy(Actor actor) {
    ...
    if (registros.size() > 0) {
      ...
      for (int i = 0; i < registros.size(); i++) {
        ...
        numeroCuentaBanco =
            StringManager.ultimosCaracteres(
                filaRegistro.get("numero_tarjeta").toString().trim(), 4);
        documentoClienteBanco =
            StringManager.formatoDocumento(filaRegistro.get("documento").toString().trim());
        ...
        for (int j = 0; j < producto.size(); j++) {
          ...
          if (numeroCuentaApp.equals(numeroCuentaBanco)) {
            ...
            resultadoRegistro =
                validarCampo(
                    "NUMERO DE TARJETA", numeroCuentaBanco, numeroCuentaApp, resultadoDato);
            resultadoRegistro =
                validarCampo(
            ...
            resultadoRegistro =
                validarCampo(
                    "FRANQUICIA DE TARJETA", franquiciaBanco, franquiciaApp, resultadoDato);
          }
        }
      }
      ...
      Serenity.recordReportData()
          .withTitle("Verificacion Back Consolidado Tarjetas")
