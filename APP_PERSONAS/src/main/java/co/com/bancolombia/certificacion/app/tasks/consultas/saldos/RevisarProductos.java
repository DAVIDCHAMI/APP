...
public class RevisarProductos implements Task {
  ...
  private String consultarSaldo(Actor actor, String tipoCuenta, String numeroCuenta) {
    ...
    else return LBL_SALDO_MOVIMIENTOS.of(tipoCuenta, numeroCuenta).resolveFor(actor).getText();
  }

  public static Performable enSaldosMovimientos(
      ProductoBuilder productoBuilder, String opcionCategoria) {
  ...
  public <T extends Actor> void performAs(T actor) {
    ...
    for (int iterador = 0; iterador <= cadena.length; iterador++) {
      if (elementoPresente(
          actor, CUENTA_ESPECIFICA_PRODUCTO.of(tipoCuenta[iterador], numeroCuenta[iterador]))) {
        ...
        listaProductos.add(
            elProducto()
                ...
                .conSaldo(
                    saldo()
                        .conSaldoDisponible(
                            consultarSaldo(actor, tipoCuenta[iterador], numeroCuenta[iterador]))
                        .build())
                .build());
        tieneProducto = true;
      } else {
