    ...
    listaCamposEprepago.add(
        eprepago()
            .conSaldoDisponible(
                saldo()
                    .conSaldoDisponible(
                        TXT_CAMPOS_ACTIVACION_EPREPAGO
                            .of(SALDO_DISPONIBLE)
                            .resolveFor(actor)
                            .getText())
                    .build())
            .conNumeroTarjeta(
                TXT_CAMPOS_ACTIVACION_EPREPAGO.of(TARJETA_MASTERCARD).resolveFor(actor).getText())
            .conFechaVencimiento(
                TXT_CAMPOS_ACTIVACION_EPREPAGO.of(FECHAVENCIMIENTO).resolveFor(actor).getText())
            .conCvc(TXT_CAMPOS_ACTIVACION_EPREPAGO.of(CODIGOSEGURIDAD).resolveFor(actor).getText())
            .build());
