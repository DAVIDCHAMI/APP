...
import co.com.bancolombia.certificacion.app.interactions.scroll.ScrollVisible;
...
public class SeleccionarOpcion implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                ScrollVisible.hastaTargetVisible(OPT_SUB_MENU.of(tipoTransaccion.getSubMenu())),
                ...
                Check.whether(!"".equals(tipoTransaccion.getTercerNivel()))
                        .andIfSo(
                                Check.whether(Visibility.of(OPT_TERCER_NIVEL.of(tipoTransaccion.getTercerNivel())).viewedBy(actor).asBoolean())
                                        .andIfSo(
                                                ScrollVisible.hastaTargetVisible(OPT_TERCER_NIVEL.of(tipoTransaccion.getTercerNivel())),
                                                Click.on(OPT_TERCER_NIVEL.of(tipoTransaccion.getTercerNivel()))
                                        ).otherwise(
                                        ScrollVisible.hastaTargetVisible(OPT_SUB_MENU.of(tipoTransaccion.getTercerNivel())),
