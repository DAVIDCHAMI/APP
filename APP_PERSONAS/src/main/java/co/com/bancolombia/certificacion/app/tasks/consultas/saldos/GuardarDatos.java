...
public class GuardarDatos implements Task {
  ...
  public <T extends Actor> void performAs(T actor) {
    ...
    if ("Android".equalsIgnoreCase(getPlatformCapability())) iterador = 0;
    else iterador = 1;
    while (Verificar.elementoPresente(
        actor, CONTENEDOR_INFORMACION_PRODUCTO.of(String.valueOf(iterador)))) {
      listaProductos.add(
          elProducto()
              .conTipoCuenta(
                  LBL_TIPO_CUENTA_SALDOS_MOVIMIENTOS
                      .of(String.valueOf(iterador))
                      .resolveFor(actor)
                      .getText())
              .conNumero(
                  LBL_NUMERO_CUENTA_SALDOS_MOVIMIENTOS
                      .of(String.valueOf(iterador))
                      .resolveFor(actor)
                      .getText())
              .conSaldo(
                  saldo()
                      .conSaldoDisponible(
                          LBL_SALDO_DISPONIBLE_SALDOS_MOVIMIENTOS
                              .of(String.valueOf(iterador))
                              .resolveFor(actor)
                              .getText())
                      .build())
