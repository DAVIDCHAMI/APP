...
public class VerificarInscripcionClaveDinamica implements Question<Boolean> {
  ...
  public Boolean answeredBy(Actor actor) {
    ...
    return (Visibility.of(LNL_COMPROBANTE_CLAVE_DINAMICA).viewedBy(actor).asBoolean()
        && Visibility.of(
                LBL_VERIFICACION_CLAVE_DINAMICA.of(
                    NOMBRE_PERSONALIZADO_CLAVE_DINAMICA, nombrePersonalizado))
            .viewedBy(actor)
            .asBoolean()
        && Visibility.of(LBL_VERIFICACION_CLAVE_DINAMICA.of(CORREO_CLAVE_DINAMICA, correo))
            .viewedBy(actor)
            .asBoolean()
        && Visibility.of(LBL_VERIFICACION_CLAVE_DINAMICA.of(TIPO_CORREO_CLAVE_DINAMICA, tipoCorreo))
            .viewedBy(actor)
            .asBoolean());
  }
