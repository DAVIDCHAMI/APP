  ...
  public Boolean answeredBy(Actor actor) {
    ...
    if (!registros.isEmpty()) {
      ...
      Double valorDisponible =
          Double.parseDouble(registros.get(0).get("sdsdodsp").toString().trim());
      ...
      resultadoDato =
          validarCampo(
              "TIPO CUENTA",
              registros.get(0).get("sdtipocta").toString().trim(),
              tipoCuentaLetra(producto.getTipo()),
              resultadoDato);
      resultadoDato =
          validarCampo(
              "CUENTA",
              registros.get(0).get("sdcuenta").toString().trim(),
              producto.getNumero().replace("-", ""),
              resultadoDato);
      resultadoDato =
          validarCampo(
              "SALDO DISPONIBLE",
              registros.get(0).get("sdsdodsp").toString().trim(),
              saldoDisponibleFront,
              resultadoDato);
      resultadoDato =
          validarCampo(
              "SALDO EN CANJE",
              registros.get(0).get("sdfltdsp").toString().trim(),
              saldoCanjeFront,
              resultadoDato);
      ...
      Serenity.recordReportData()
          .withTitle("Verificacion Back Detalle Depositos")
