    ...
    if (!registros.isEmpty()) {
      ...
      for (int i = 0; i < registros.size(); i++) {
        ...
        String numeroCuentaBanco =
            filaRegistro.get("encco1").toString().trim()
                + filaRegistro.get("encnum").toString().trim();
        ...
        for (int j = 0; j < producto.size(); j++) {
          String numeroCuentaApp =
              producto.get(j).getNumero().trim().substring(0, 4)
                  + ultimosCaracteres(producto.get(j).getNumero().trim(), 3);
          String tipoCuentaBancoApp =
              FondoConstantes.BIBLIOTECA_POR_CODIGO.get(producto.get(j).getTipo().trim());
          ...
          if (numeroCuentaApp.equals(numeroCuentaBanco)
              && tipoCuentaBancoApp.equals(tipoCuentaBanco)) {
            int k = j + 1;
            resultadoDato = true;
            resultadoRegistro = true;
            resultadoRegistro =
                validarCampo("CUENTA # " + k, numeroCuentaBanco, numeroCuentaApp, resultadoDato);
            resultadoRegistro =
                validarCampo(
                    "TIPO CUENTA # " + k, tipoCuentaBanco, tipoCuentaBancoApp, resultadoDato);
            resultadoRegistro =
                validarCampo(
                    "SALDO DISPONIBLE # " + k,
                    saldoDisponibleBanco,
                    saldoDisponibleApp,
                    resultadoDato);
          }
        }
      ...
      Serenity.recordReportData()
          .withTitle("Verificacion Back Consolidado Depositos")
