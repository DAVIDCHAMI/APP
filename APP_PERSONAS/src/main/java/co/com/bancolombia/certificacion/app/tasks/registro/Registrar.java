...
public class Registrar implements Task {
  ...
  public <T extends Actor> void performAs(T actor) {
    ...
    actor.attemptsTo(Click.on(LBL_INICIA_REGISTRO), Click.on(BTN_CANCELAR_TRANSACCION));
    if (Verificar.elementoVisible(actor, BTN_SI_CANCELAR_REGISTRO)) {
      actor.attemptsTo(Click.on(BTN_SI_CANCELAR_REGISTRO));
    }
    actor.attemptsTo(
        Click.on(OPT_MENU_PRINCIPAL.of(INICIO)),
        IniciarSesion.con(
            configuracionTransaccion.getUsuario().getNombreUsuario(),
            configuracionTransaccion.getUsuario().getClave()));
  }
