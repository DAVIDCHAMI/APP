...
public class ConInicioSesion extends GenerarQR {
  ...
  public <T extends Actor> void performAs(T actor) {
    actor.attemptsTo(
        // Saltar.onBoarding(),
        WaitUntil.the(LNK_SIGUIENTE, isEnabled()), Click.on(LNK_SIGUIENTE));
    if (Verificar.elementoVisible(actor, LBL_VERIFICACION.of(CUENTAS))) {
      actor.attemptsTo(
          WaitUntil.the(
              BTN_PRODUCTO_ORIGEN.of(
                  datos.getProductoOrigen().getTipo(), datos.getProductoOrigen().getNumero()),
              isPresent()),
    ...
    actor.attemptsTo(
        Check.whether("".equals(datos.getMonto()))
            .andIfSo(Click.on(BTN_SIN_VALOR))
            .otherwise(
        ...
        Validar.carga());
    if ("".equals(datos.getMonto())) datos.setMonto(SIN_VALOR);
    else datos.setMonto(darFormato(datos.getMonto()));
