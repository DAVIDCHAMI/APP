...
public class ConsultarFacturaProgramadas implements Task {
  ...
  public <T extends Actor> void performAs(T actor) {
    actor.attemptsTo(Click.on(OPT_PROGRAMADAS));
    int iterador = 0;
    List<Factura> listaFactura = new ArrayList<>();
    while (Verificar.elementoVisible(
        actor, LBL_DESCRIPCION_PROGRAMADA_FACTURA.of(String.valueOf(iterador)))) {
      listaFactura.add(
          factura()
              .conDescripcionFactura(
                  LBL_DESCRIPCION_PROGRAMADA_FACTURA
                      .of(String.valueOf(iterador))
                      .resolveFor(actor)
                      .getText())
              .conEmpresaServicio(
                  LBL_EMPRESA_CONVENIO_FACTURA_PROGRAMADA
                      .of(String.valueOf(iterador))
                      .resolveFor(actor)
                      .getText())
              .conFechaFactura(
                  LBL_FECHA_FACTURA_PROGRAMADA
                      .of(String.valueOf(iterador))
                      .resolveFor(actor)
                      .getText())
              .conValor(
                  LBL_VALOR_FACTURA_PROGRAMADA
                      .of(String.valueOf(iterador))
                      .resolveFor(actor)
                      .getText())
              .build());
