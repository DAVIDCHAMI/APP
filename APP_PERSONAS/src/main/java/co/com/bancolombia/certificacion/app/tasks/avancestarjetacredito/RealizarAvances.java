...
public class RealizarAvances implements Task {
    ...
    public RealizarAvances(String tipoAvance, TarjetaCredito tarjetaCredito){
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
        Check.whether("realiza avance".equals(tipoAvance)).andIfSo(
                        Click.on(BTN_TARJETA_CREDITO),
                        WaitUntil.the(LST_CONTENEDOR_TARJETAS, isVisible())
            )
        );
            int size= LST_CONTENEDOR_TARJETAS.resolveAllFor(actor).size();
