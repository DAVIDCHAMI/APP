...
public class ConsultarSaldosConsolidadosCreditos implements Question<Boolean> {
  ...
  private static final Logger LOGGER =
      LogManager.getLogger(ConsultarSaldosConsolidadosCreditos.class);
  ...
  public Boolean answeredBy(Actor actor) {
    ...
    if (!registros.isEmpty()) {
      ...
      resultadoDato =
          validarCampo(
              "CUENTA",
              registros.get(0).get("numero_credito").toString().trim(),
              listaProductos.get(0).getNumero().replace("-", ""),
              resultadoDato);
      ...
      Serenity.recordReportData()
          .withTitle("Verificacion Back saldos creditos")
