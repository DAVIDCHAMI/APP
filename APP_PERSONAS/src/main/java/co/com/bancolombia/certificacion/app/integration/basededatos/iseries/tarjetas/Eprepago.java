...
public class Eprepago {
  ...
  public static boolean verificoEstadoDeLaActivacionDeLaEprepago(Actor actor) {
    ...
    ConfiguracionTransaccion datosPrincipales = actor.recall(MODELO_DATOS_TRANSACCION);
    co.com.bancolombia.backend.modelo.usuario.Usuario usuario =
        new co.com.bancolombia.backend.modelo.usuario.Usuario();
    ...
    try {
      stateEprepago =
          ePrepago.consultarDetalleEprepago(usuario, AdministradorConstante.CODIGO_BANCO_EPREPAGO);
      if (stateEprepago != null
          && AdministradorConstante.ESTADO_TARJETA_ACTIVA.equals(stateEprepago.getEstado())) {
  ...
  public static boolean verificoEstadoDeLaInactivacionDeLaEprepago(Actor actor) {
    ...
    try {
      stateEprepago =
          ePrepago.consultarDetalleEprepago(usuario, AdministradorConstante.CODIGO_BANCO_EPREPAGO);
      if (stateEprepago != null
          && AdministradorConstante.ESTADO_TARJETA_INACTIVA.equals(stateEprepago.getEstado())) {
  ...
  public static TarjetaEPrepago verificoElRegistroDeLaEprepago(Actor actor) {
    ...
    try {
      validarRegistroEprepago =
          ePrepago.verifcarRegistroEprepago(datosPrincipales.getUsuario().getNumeroDocumento());
    } catch (SQLException e) {
      LOGGER.error(e.getMessage(), e);
    }
  ...
  public static boolean verificoRegistroDeLaEprepago(Actor actor) {
    ...
    try {
      validarRegistroEprepago =
          ePrepago.verificarRegistroEprepagoTarj(
              datosPrincipales.getUsuario().getNumeroDocumento());
    } catch (SQLException e) {
      LOGGER.error(e.getMessage(), e);
    }
    return validarRegistroEprepago;
  ...
  public static boolean validarCreditoEprepago(
      String saldoAntes, String saldoDespues, String monto, String orientacion) {
    ...
    return AdministradorConstante.ACIERTO.equalsIgnoreCase(orientacion)
        ? saldoAnterior.add(valorTransaccion).equals(saldoPosterior)
        : saldoAnterior.equals(saldoPosterior);
  }

  public static boolean validarDebitoEprepago(
      String saldoAntes, String saldoDespues, String monto, String orientacion) {
    BigDecimal saldoAnterior = new BigDecimal(saldoAntes);
    BigDecimal saldoPosterior = new BigDecimal(saldoDespues);
    BigDecimal valorTransaccion = new BigDecimal(monto);
    return AdministradorConstante.ACIERTO.equalsIgnoreCase(orientacion)
        ? saldoAnterior.subtract(valorTransaccion).equals(saldoPosterior)
        : saldoAnterior.equals(saldoPosterior);
  }
}
