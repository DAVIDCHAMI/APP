    ...
    if (("Android").equalsIgnoreCase(platform)) {
      ...
      for (int i = 0; i < cadena.length(); i++) {
        ...
        int[] codigoLetra = {
          ...
          51, 52, 53, 54, 77, 56, 8, 62, 9, 10, 11, 12, 13, 14, 15, 16, 7
        };
        for (int j = 0; j <= 38; j++) {
          String letraA = String.valueOf(letras[j]);
          if (letraTeclado.equalsIgnoreCase(letraA) && mayus) {
            driver.pressKeyCode(codigoLetra[j], AndroidKeyMetastate.META_CAPS_LOCK_ON);
      ...
      actor.attemptsTo(Type.theValue(cadena).into(TXT_SEGUNDA_CLAVE));
    }
  }
