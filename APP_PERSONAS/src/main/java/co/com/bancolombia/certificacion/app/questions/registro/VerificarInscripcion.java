...
public class VerificarInscripcion implements Question<Boolean> {
  ...
  public Boolean answeredBy(Actor actor) {
    ...
    String numeroProducto =
        obtenerValorEntero(Text.of(LBL_NUMERO_PRODUCTO).viewedBy(actor).asString());
    return (numeroProducto.equals(inscripcion.getProducto().getNumero())
        && Visibility.of(IMG_CONFIRMACION).viewedBy(actor).asBoolean()
        && Visibility.of(LBL_TIPO_PRODUCTO.of(inscripcion.getProducto().getTipo()))
            .viewedBy(actor)
            .asBoolean()
        && Visibility.of(LBL_TIPO_DOCUMENTO.of(inscripcion.getUsuario().getTipoDocumento()))
            .viewedBy(actor)
            .asBoolean()
        && Visibility.of(LBL_NUMERO_DOCUMENTO.of(inscripcion.getUsuario().getNumeroDocumento()))
            .viewedBy(actor)
            .asBoolean()
        && Visibility.of(LBL_BANCO.of(inscripcion.getNombreBanco().toLowerCase()))
            .viewedBy(actor)
            .asBoolean());
  }
