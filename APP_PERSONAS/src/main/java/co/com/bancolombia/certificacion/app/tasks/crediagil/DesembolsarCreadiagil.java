    ...
    if (Verificar.elementoVisible(actor, LBL_VERIFICACION.of(CUENTAS))) {
      actor.attemptsTo(
          RealizarScroll.hastaPosicionDeTarget(
              OPT_PRODUCTO_ORIGEN.of(productoDesembolso.getTipo(), productoDesembolso.getNumero())),
          Click.on(
              OPT_PRODUCTO_ORIGEN.of(
                  productoDesembolso.getTipo(), productoDesembolso.getNumero())));
    }
    actor.attemptsTo(Enter.theValue(valorDesembolsar).into(TXT_VALOR_DESEMBOLSAR), Click.on(FOCO));
  }

  public static DesembolsarCreadiagil fueraRango(
      String valorDesembolsar, ProductoBuilder productoDesembolso) {
