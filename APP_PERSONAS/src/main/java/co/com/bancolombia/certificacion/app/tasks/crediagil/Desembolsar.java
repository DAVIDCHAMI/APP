...
public class Desembolsar implements Task {
    ...
    public Desembolsar(String valorDesembolsar, Producto productoDebitar) {
    ...
    public <T extends Actor> void performAs(T actor) {
        if (Verificar.elementoVisible(actor, LBL_VERIFICACION.of(CUENTAS))) {
            actor.attemptsTo(
                    RealizarScroll.hastaTargetVisible(OPT_PRODUCTO_ORIGEN.of(productoDebitar.getTipo(), productoDebitar.getNumero())),
    ...
    public static Desembolsar crediagil(String valorDesembolsar, ProductoBuilder productoDebitar) {
        return instrumented(Desembolsar.class, valorDesembolsar, productoDebitar.build());
    }
}
