...
public class RealizarTransferencia implements Task {
  ...
  public RealizarTransferencia(
      Transferencia transferencia, String opcionProductoOrigen, String opcionProductoDestino) {
  ...
  public <T extends Actor> void performAs(T actor) {
    actor.attemptsTo(
        Check.whether(OPCION_INVERSIONES.equalsIgnoreCase(opcionProductoOrigen))
            .andIfSo(Click.on(BTN_INVERSIONES))
            .otherwise(Click.on(BTN_CUENTAS)),
        SeleccionarProducto.desdeSaldosMovimientos(
            transferencia.getProductoOrigen().getTipo(),
            transferencia.getProductoOrigen().getNumero(),
            CUENTA_ESPECIFICA_PRODUCTO),
        Enter.theValue(transferencia.getMonto()).into(TXT_VALOR_TRANSFERENCIA),
        Click.on(TXT_FOCO),
        Click.on(LNK_SIGUIENTE),
        Check.whether(
                (transferencia.getTipoTransferencia()).contains(TRANSFERIR_PRODUCTOS_NO_INSCRITOS))
            .andIfSo(
                Click.on(OPT_TIPO_TRANSFERENCIA.of(TRANSFERIR_PRODUCTOS_NO_INSCRITOS)),
                Enter.theValue(transferencia.getProductoDestino().getNumero())
                    .into(TXT_NUMERO_CUENTA_DESTINO),
                Click.on(TXT_FOCO),
            ...
            .otherwise(
                Check.whether(
                        transferencia
                            ...
                            .contains(TRANSFERIR_PRODUCTOS_PROPIOS_INSCRITOS))
                    .andIfSo(
                        Click.on(OPT_TIPO_TRANSFERENCIA.of(TRANSFERIR_PRODUCTOS_PROPIOS_INSCRITOS)),
                        Check.whether(INSCRITOS.contains(opcionProductoDestino))
                            .andIfSo(
                                Click.on(BTN_TIPO_PRODUCTO_DESTINO.of(opcionProductoDestino)),
                                Click.on(
                                    BTN_PRODUCTO_INSCRITO.of(
                                        transferencia.getProductoDestino().getNumero())))
                            .otherwise(
                                SeleccionarProducto.desdeSaldosMovimientos(
                                    transferencia.getProductoDestino().getTipo(),
                                    transferencia.getProductoDestino().getNumero(),
                                    CUENTA_ESPECIFICA_PRODUCTO)))
                    .otherwise(
  ...
  public static RealizarTransferencia conInfo(
      TransferenciaBuilder transferencia,
      String opcionProductoOrigen,
      String opcionProductodestino) {
    return instrumented(
        RealizarTransferencia.class,
        transferencia.build(),
        opcionProductoOrigen,
        opcionProductodestino);
  }
}
